document.addEventListener('DOMContentLoaded', function () {
  ...
  function clearFieldState(field) {
    ...
    const errorEl = field.parentElement && field.parentElement.querySelector('.field-error-msg');
    if (errorEl) errorEl.textContent = '';
  }
  ...
  function validateField(field) {
    ...
    if (field.minLength && field.minLength > 0 && value.length < field.minLength) {
      setFieldError(field, field.dataset.errorMinlength || 'Input is too short.');
      return false;
    }

    if (field.maxLength && field.maxLength > 0 && value.length > field.maxLength) {
      setFieldError(field, field.dataset.errorMaxlength || 'Input is too long.');
      return false;
    }
  ...
  function showFormMessage(form, message, isSuccess) {
    ...
    msgEl.classList.toggle('form-status-msg--success', isSuccess);
    msgEl.classList.toggle('form-status-msg--error', !isSuccess);
  ...
  document.body.addEventListener('submit', function (e) {
    ...
    if (!isFormValid) {
      ...
      if (firstError) {
        ...
        window.scrollTo({ top: elementTop - navbarHeight - 20, behavior: 'smooth' });
      }
      showFormMessage(form, form.dataset.errorGeneral || 'Please fix the errors above and try again.', false);
      return;
    }
    ...
    fetch(action, {
      ...
      .then(function (response) {
        if (response.ok) {
          showFormMessage(form, form.dataset.successMessage || 'Thank you! Your message has been sent.', true);
          form.reset();
          fields.forEach(function (field) { clearFieldState(field); });
        } else {
      ...
      .catch(function (err) {
        showFormMessage(form, form.dataset.errorMessage || 'Something went wrong. Please try again later.', false);
  ...
  const animatableElements = document.querySelectorAll(ANIMATION_CLASSES.join(', '));
    ...
    const animationObserver = new IntersectionObserver(function (entries) {
      entries.forEach(function (entry) {
        if (entry.isIntersecting) {
          ...
          const delayMs = parseFloat(delay) * (delay.includes('s') && !delay.includes('ms') ? 1000 : 1);
  ...
  const staggerContainers = document.querySelectorAll('[data-stagger], .stagger-children');
  ...
  if (staggerContainers.length > 0) {
      ...
      entries.forEach(function (entry) {
        if (entry.isIntersecting) {
          ...
          Array.from(children).forEach(function (child, index) {
            setTimeout(function () {
              child.classList.add('animated', 'is-visible');
              child.classList.remove('animation-ready');
            }, index * staggerDelay);
          });
    ...
    staggerContainers.forEach(function (container) {
      Array.from(container.children).forEach(function (child) {
        child.classList.add('animation-ready');
      });
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "LU (LOVE YOU) משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    (function() {
        ...
        function initContactForm() {
            ...
            try {
                ...
                const payload = {
                    websiteId: 'f3bc1584-ad9e-4414-88e0-437247e44148',
                    name: name,
                    email: email,
                    subject: subject,
                    message: message,
                ...
                if (result.success) {
                    ...
                    if (result.thankYouPagePath && result.ticketNumber) {
                        ...
                        if (isPreviewMode && window.ZAPPY_CONFIG) {
                            ...
                            const websiteId = window.ZAPPY_CONFIG.websiteId || 'f3bc1584-ad9e-4414-88e0-437247e44148';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'f3bc1584-ad9e-4414-88e0-437247e44148';
